 ...
 * Now supports hash-based beam scattering and alias compensation.
 *
 * Responsibilities:
 ...
 *   - Implement hash-based beam scattering for energy conservation
 *
 * API Methods:
 *   - setGridParameters(...) : choose circular kernel vs grid sampling
 *   - processMask(inputMask) : Eigen::Tensor<DataType,3> focused buffer
 *   - scatterBeam(...) : hash-based beam scattering and alias remapping
 */
...
public:
     ...
     * scatterBeam
     * -----------
     * Implements hash-based beam scattering and alias remapping.
     *
     * @param beamHash  Hash table of beam entries
     * @param kernel    Spatial influence envelope
     * @return Updated hash table with scattered energy
     */
    std::unordered_map<int, std::tuple<Eigen::Vector3f, float>> scatterBeam(
        const std::unordered_map<int, std::tuple<Eigen::Vector3f, float>>& beamHash,
        const Eigen::Tensor<DataType,3>& kernel) const;
