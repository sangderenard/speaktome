...
def describe_tick(graph: FluxGraph, tokenizer) -> None:
    ...
    print(f"  best path (score={score:.2f}): {text!r}")


def run_demo(seed_text: str, ticks: int, budget: int, branch_factor: int) -> None:
    print("Loading GPT-2 ...")
    t0 = time.time()
    scorer_obj = Scorer()
...
def main() -> None:
    ...
    run_demo(args.seed, args.ticks, args.budget, args.branch)
