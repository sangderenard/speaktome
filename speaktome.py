...
def main(raw_args=None, allow_retry=True):
    ...
    # Wrap the run to allow falling back to CPU on GPU failures. Some
    # CUDA errors leave the runtime unstable, so this is a best-effort
    # attempt and may still fail without a full process restart.
    try:
        run_once()
        ...
        if allow_retry and not args.human_control and config.DEVICE.type == "cuda":
            ...
            try:
                ...
                # NOTE: device-side asserts can leave the CUDA runtime in
                # an unrecoverable state. If clearing the cache fails we
                # log and continue; a full restart may still be required.
                print(f"Warning: failed to empty CUDA cache: {cache_exc}")
            run_once()
        else:
            raise
