...
logging.basicConfig(
    ...
    handlers=[
        ...
        # You can add more handlers here if needed (e.g., for different log levels)
    ],
)
...
def draw_graph(screen, positions, edge_index, node_states, error_magnitudes, cmap='jet'):
    """
    Draws the graph network on the Pygame screen.

    Args:
        screen (pygame.Surface): The Pygame screen surface.
        positions (dict): Dictionary mapping node indices to (x, y) positions.
        edge_index (torch.Tensor): Tensor containing edge connections.
    ...
    for edge in edge_index.t().tolist():
        ...
        pygame.draw.line(screen, EDGE_COLOR, src_pos, dst_pos, 1)
    ...
    for node_idx, pos in positions.items():
        ...
        # Render node ID
        text_surface = FONT.render(str(node_idx), True, TEXT_COLOR)
        screen.blit(text_surface, (pos[0]*.001 - NODE_RADIUS, pos[1]*.001 - NODE_RADIUS - 15))

    pygame.display.flip()  # Update the full display Surface to the screen

def generate_grid_positions(num_nodes, width, height, margin=50):
    """
    ...
    spacing_x = (width - 2 * margin) / (grid_size - 1) if grid_size > 1 else width / 2
    spacing_y = (height - 2 * margin) / (grid_size - 1) if grid_size > 1 else height / 2
    ...
    for node in range(num_nodes):
        ...
        positions[node] = [x, y]  # Use list for mutability

    return positions

def determine_leading_dimensions(state_matrix, middle_node_indices, n_components=2):
    """
    Determines the leading dimensions using PCA on middle nodes.

    Args:
        state_matrix (torch.Tensor): State matrix of all nodes.
        middle_node_indices (list): List of middle node indices.
        n_components (int): Number of principal components to return.

    Returns:
        np.ndarray: Principal components vectors.
    """
    middle_states = state_matrix[middle_node_indices].numpy()
    pca = PCA(n_components=n_components)
    pca.fit(middle_states)
    return pca.components_  # Shape: (n_components, state_vector_dim)

def update_node_positions(positions, state_matrix, leading_dims, grid_spacing=50):
    """
    Updates node positions based on their state vectors and leading dimensions.

    Args:
        positions (dict): Current positions of nodes.
        state_matrix (torch.Tensor): State matrix of all nodes.
        leading_dims (np.ndarray): Leading dimensions from PCA.
        grid_spacing (float): Scaling factor for position deviation.

    Returns:
        dict: Updated positions of nodes.
    """
    for node_idx, pos in positions.items():
        state = state_matrix[node_idx].numpy()
        # Project state onto leading dimensions
        deviation = leading_dims @ state  # Shape: (n_components,)
        # Update position with deviation scaled by grid_spacing
        pos[0] += grid_spacing * deviation[0]
        pos[1] += grid_spacing * deviation[1]
    return positions

def compute_error_magnitudes(node_states):
    """
    Computes error magnitudes for all nodes.
    ...
    """
    # Example: Sum of absolute state values as error magnitude
    return torch.sum(torch.abs(node_states), dim=1)
...
def prune_graph(graph_data, simulators, positions):
    ...
    # Assuming 'bounds' is a binary mask indicating active dimensions
    bounds = graph_data.edge_attr["bounds"]
    # Identify edges where all bounds are zero
    ...
    if edges_to_prune.sum().item() > 0:
        ...
        mask = ~edges_to_prune
        # Prune edge_index and edge_attr
        ...
        for key in graph_data.edge_attr:
            if isinstance(graph_data.edge_attr[key], torch.Tensor):
                ...
                # Handle non-tensor attributes if any
                pass
    else:
        logging.info("No edges to prune based on bounds.")
    
    # Step 2: Remove isolated nodes (nodes with no connected edges)
    # remove_isolated_nodes returns the new edge_index and node_mask
    ...
    if nodes_removed > 0:
        ...
        # Update graph_data.edge_index
        graph_data.edge_index = new_edge_index
        # Update node attributes (x)
        graph_data.x = graph_data.x[node_mask]
        # Update other node-related data structures
        # Update simulators
...
if __name__ == "__main__":
    ...
    def generate_random_config(state_vector_dim, max_force=100000, max_debt=1000):
        bounds = torch.tensor([random.choice([1, 1]) for _ in range(state_vector_dim)], dtype=TENSOR_TYPE)
        force_limits = torch.tensor([random.uniform(max_force*.5, max_force) for _ in range(state_vector_dim)], dtype=TENSOR_TYPE)
        exchange_rates = torch.tensor([random.uniform(.8, 1.2) for _ in range(state_vector_dim)], dtype=TENSOR_TYPE)
    ...
    positions = generate_grid_positions(NUM_NODES, WINDOW_WIDTH, WINDOW_HEIGHT)

    # Identify middle node indices (central nodes)
    grid_size = int(np.ceil(np.sqrt(NUM_NODES)))
    center = grid_size // 2
    middle_node_indices = [i for i in range(NUM_NODES)
                           if (i // grid_size) == center or (i % grid_size) == center]
    ...
    for iteration in range(NUM_ITERATIONS):
        ...
        # Prune the graph: remove disconnected edges and isolated nodes
        ...
        if not continue_simulation:
            ...
            break
        # Update node positions based on leading dimensions
        leading_dims = determine_leading_dimensions(graph_data.x, middle_node_indices)
        positions = update_node_positions(positions, graph_data.x, leading_dims)

        # Compute error magnitudes for color mapping
        error_magnitudes = compute_error_magnitudes(graph_data.x)

        # Update visualization
        #draw_graph(screen, positions, graph_data.edge_index, graph_data.x, error_magnitudes)
