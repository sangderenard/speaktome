...
class Material:
    """
    Represents a tensor or data structure manipulated by tools.
    """
    ...
    def transform(self, transformation):
        """
        Apply a transformation function to the material.
...
class Paper(Material):
    def __init__(self, name: str, properties: dict):
        super().__init__(data=torch.zeros((3, 500, 500)), metadata=properties)
...
class GrandPress(Machine):
    ...
    def operate(self, material: Material, glyph_tensor: torch.Tensor, gradient_tensor: torch.Tensor, mask_tensor: torch.Tensor = None):
        ...
        grand_press = GrandPressInstance()  # Instance from GrandPrintingPress
        processed_tensor = grand_press.press(material.data, glyph_tensor, gradient_tensor, mask_tensor)
...
class Digitizer(Machine):
    ...
    def operate(self, material: Material):
        ...
        digitizer = DigitizerInstance()  # Instance of Digitizer
        texture_id = digitizer.tensor_to_texture(material.data)
        return texture_id
...
class InkWorker(Worker):
    ...
    def perform_task(self):
        ...
        # Example: Apply gradient using GradientMaker tool
        ...
        grand_press.operate(ink_material, glyph_tensor=torch.rand(3, 100, 100), gradient_tensor=gradient)
...
class PaperWorker(Worker):
    ...
    def perform_task(self):
        ...
        tool_coordinator, material_coordinator, machine_coordinator = self.coordinator_trio
        # Example: Assemble glyphs using GlyphAssembler tool
...
class PressOperator(Worker):
    ...
    def perform_task(self):
        ...
        tool_coordinator, material_coordinator, machine_coordinator = self.coordinator_trio
        # Example: Retrieve a paper material and apply ink
        paper_material = material_coordinator.allocate_material((3, 500, 500))
...
class GrandPress(Machine):
    def __init__(self, name: str, tools: list):
        super().__init__(name, tools)
        self.grand_press_instance = GrandPressInstance()  # Instance from GrandPrintingPress
...
class Digitizer(Machine):
    def __init__(self, name: str, tools: list):
        super().__init__(name, tools)
        self.digitizer_instance = DigitizerInstance()  # Instance of Digitizer
...
def print_shop_demo():
    ...
    ruler = Ruler(dpi=300)
    granulator = Granulator(base_unit=1)
    ...
    black_pigment = Pigment(
        ...
        spectrum=torch.tensor([0.0, 0.0, 0.0], dtype=torch.float32)
    )
    ...
    # Add workers
    ink_worker = InkWorker("Alice", ["Grand Press"], ["Glyph Assembler"], (shop.tool_coordinator, shop.material_coordinator, shop.machine_coordinator))
    paper_worker = PaperWorker("Bob", [], ["Glyph Assembler"], (shop.tool_coordinator, shop.material_coordinator, shop.machine_coordinator))
    press_operator = PressOperator("Charlie", ["Grand Press"], [], (shop.tool_coordinator, shop.material_coordinator, shop.machine_coordinator))
...
# -----------------------------------------------------------------------------
# Supporting Classes (Ruler, Granulator, PaperMaker, Inkmaker, Pigment, Xuanzhi)
# -----------------------------------------------------------------------------
...
class Inkmaker:
    ...
    **Inkmaker**
    ...
    def simulate_bleed(self, ink_tensor: torch.Tensor, bleed_factor: float = 0.05):
        ...
        kernel_size = int(bleed_factor * min(ink_tensor.shape[-2:]))
        if kernel_size < 1:
            kernel_size = 1
        kernel = torch.ones((kernel_size, kernel_size)) / kernel_size**2
