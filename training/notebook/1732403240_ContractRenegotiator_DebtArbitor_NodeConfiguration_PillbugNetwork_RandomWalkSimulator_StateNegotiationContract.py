...
def create_graph_data(simulators, configs):
    ...
    for edge in edge_index.t():
        ...
        src, dst = edge[0].item(), edge[1].item()
        src_type = 1.3 if configs[src].exchange_type == "float" else 1.0
        dst_type = 1.3 if configs[dst].exchange_type == "float" else 1.0
...
if __name__ == "__main__":
    ...
    def generate_random_config(state_vector_dim, max_force=100000, max_debt=1000):
        ...
        return NodeConfiguration(bounds, force_limits, exchange_rates, debt_threshold, exchange_type)
