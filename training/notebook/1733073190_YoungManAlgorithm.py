...
from bitmaskinterpreter import BitmaskInterpreter
logging.basicConfig(level=logging.INFO, format="%(message)s")


class YoungManAlgorithm:
    def __init__(self, geometry="cube", density=1.0, save_dir="geometry_maps", device="cuda", precision=torch.float64, jitter_enabled=False, jitter_seed=None, jitter_strength = (torch.pi, torch.pi/2, 0), micro_jitter=False, enforce_req_grad=True, use_new_bitmask_logic=True, bitmask_interpreter=None):
        ...
        self.use_new_bitmask_logic = use_new_bitmask_logic  # Flag to toggle behavior
        ...
        init_geometry = self.geometry_maps[self.current_geometry][self.density]
        # Redirect methods to CompositeGeometry
        # Create or assign BitmaskInterpreter
        if bitmask_interpreter is not None:
            self.bitmask_interpreter = bitmask_interpreter
        else:
            self.bitmask_interpreter = self._initialize_bitmask_interpreter(init_geometry)

    def _initialize_bitmask_interpreter(self, init_geometry):
        
        
        return BitmaskInterpreter(
                vertex_count=init_geometry.vertex_count,  # Example value; replace with actual count
                edge_pairs=init_geometry.edge_pairs,  # Replace with actual edges
                vertex_offsets=init_geometry.vertex_offsets,  # Replace with actual offsets
                device=self.device,
                precision=self.precision,
                cache=self.geometry_maps,
