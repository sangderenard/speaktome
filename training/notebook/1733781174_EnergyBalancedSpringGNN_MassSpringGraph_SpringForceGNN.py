...
############################
def node_configuration_builder(config_type="particles", num_nodes=10, device="cuda", precision=torch.float64):
    """
    ...
    node_masses = torch.ones(num_nodes, dtype=precision, device=device)  # Default masses
    node_charges = torch.zeros(num_nodes, dtype=precision,  device=device)  # Default neutral charge
    node_magnetic = torch.zeros((num_nodes, 3), dtype=precision,  device=device)  # Default no magnetic field
    node_magnetic_moment = torch.zeros((num_nodes, 3), dtype=precision,  device=device)  # Default no magnetic moment
    node_surface_quality = torch.zeros(num_nodes, dtype=precision,  device=device)  # Surface quality for plasma arcs

    if config_type == "planets":
        # Simulate massive planetary objects with gravity
        node_masses = torch.abs(torch.randn(num_nodes, dtype=precision,  device=device) * 5e24)  # Large masses
        node_charges = torch.zeros(num_nodes, dtype=precision,  device=device)  # Planets are electrically neutral
        node_magnetic = torch.randn((num_nodes, 3), dtype=precision,  device=device) * 1e-3  # Weak planetary magnetism
        node_magnetic_moment = node_magnetic * 1e2  # Proportional magnetic moment for planetary bodies
        node_surface_quality = torch.rand(num_nodes, dtype=precision,  device=device) * 0.1  # Low arc propensity for planets

    elif config_type == "particles":
        # Simulate fundamental particles with electric charge and magnetic fields
        node_masses = torch.abs(torch.randn(num_nodes, dtype=precision,  device=device) * 1e-27)  # Particle masses
        node_charges = torch.randn(num_nodes, dtype=precision,  device=device) * 1e-19  # Charges (Coulombs)
        node_magnetic = torch.randn((num_nodes, 3), dtype=precision,  device=device) * 1e-6  # Small magnetic vectors
        node_magnetic_moment = node_magnetic * 1e-4  # Proportional magnetic moment
        node_surface_quality = torch.rand(num_nodes, dtype=precision,  device=device) * 0.9 + 0.1  # High arc propensity for particles

    elif config_type == "objects":
        # Simulate everyday objects with moderate mass and optional magnetism
        node_masses = torch.abs(torch.randn(num_nodes, dtype=precision,  device=device) * 10.0)  # Moderate masses (kg)
        node_charges = torch.randn(num_nodes, dtype=precision,  device=device) * 1e-3  # Small charge distribution
        node_magnetic = torch.randn((num_nodes, 3),  dtype=precision, device=device) * 1e-3  # Optional magnetism
        node_magnetic_moment = node_magnetic * 1e0  # Moderate magnetic moment
        node_surface_quality = torch.rand(num_nodes, dtype=precision,  device=device) * 0.5 + 0.3  # Medium arc propensity
...
class MassSpringGraph:
    def __init__(self, positions, edges, node_masses, edge_params, edge_masses, edge_moments, node_magnetic_moments, node_charges, device="cuda"):
        self.device = device
        ...
        self.node_charges = node_charges.to(device)
    ...
    def compute_field_forces(self):
        ...
        for field in self.field_sources:
            ...
            # Electrostatic Forces (Coulomb's Law)
                ...
                charges = self.node_charges  # Node charges
            ...
            # Magnetic Forces (Lorentz Force)
                ...
                lorentz_force = self.node_charges.unsqueeze(1) * (E_field + qv_cross_B)
    ...
    def step(self, dt):
        subdt = 10
        subdt_span = dt / subdt        
        for i in range(subdt+1):
            this_dt = i * subdt_span
        ...
        # Compute internal forces
            ...
            self.velocities += acceleration * subdt_span
            self.positions += self.velocities * subdt_span
...
def main():
    ...
    geometry_type = args.geometry_type  # Options: "cube", "tetrahedron", etc.
    density = 1
    ...
    node_masses = torch.ones(positions.shape[0], dtype=precision, device=device) * 50.0
    edge_masses = torch.ones(len(edges), dtype=precision, device=device) * 5.0
    edge_moments = torch.ones(len(edges), dtype=precision, device=device) * 0.5

    # Edge Attributes (environmental and spring)
    rest_lengths = torch.norm(positions[edges[...,0]] - positions[edges[...,1]], dim=1) * 1.0
    spring_constants = torch.full_like(rest_lengths, 0.0)
    damping_ext = torch.full_like(rest_lengths, 0.0)
    damping_cmp = torch.full_like(rest_lengths, 0.0)
    gas_temp = torch.full_like(rest_lengths, 0.0)
    max_extent = rest_lengths * 1.2
    min_extent = rest_lengths * 0.6
    rubber_stiffness = torch.full_like(rest_lengths, 0.0)
    steel_rigidity = torch.full_like(rest_lengths, 0.0)
    ...
    system = MassSpringGraph(
        ...
        node_charges=node_charges,  # Updated from node_charges
        device=device
    )
