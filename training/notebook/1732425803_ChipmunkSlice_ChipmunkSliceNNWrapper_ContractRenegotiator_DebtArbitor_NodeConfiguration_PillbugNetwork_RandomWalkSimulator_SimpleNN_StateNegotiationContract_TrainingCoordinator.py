...
if __name__ == "__main__":
    ...
    def generate_random_config(state_vector_dim, max_force=100000, max_debt=1000):
        bounds = torch.tensor([random.choice([1, 1]) for _ in range(state_vector_dim)])
        force_limits = torch.tensor([random.uniform(max_force * 0.99, max_force) for _ in range(state_vector_dim)])
        exchange_rates = torch.tensor([random.uniform(1.01, 1.02) for _ in range(state_vector_dim)])
        debt_threshold = random.uniform(1000, max_debt)
        exchange_type = random.choice(["float", "float"])  # Randomly assign exchange type
        config = NodeConfiguration(bounds, force_limits, exchange_rates, debt_threshold, exchange_type)
        config.mass = random.uniform(1.0, 1.0)  # Add mass attribute
        return config
