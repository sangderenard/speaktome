...
class Node:
    ...
    def log_state(self, stage, tensor=None, loss=None):
        print(f"[{self.node_type.upper()}] Stage: {stage}")
        if tensor is not None:
            print(f"  Tensor stats: mean={tensor.mean().item():.4f}, std={tensor.std().item():.4f}")
        if loss is not None:
    ...
    def process_task(self, input_tensor, reference_tensor=None):
        if self.node_type == "generator":
            ...
            if reference_tensor is not None:
                ...
                return output.clone().detach(), loss.item()
            return output, None
...
class Edge:
    ...
    def forward_pass(self, data):
        print(f"[EDGE] Forwarding data from {self.source.id} to {self.target.id}")
        self.cache = data
        self.target.receive_message(data)

    def backward_pass(self):
        if self.cache is not None:
            print(f"[EDGE] Backward passing data from {self.target.id} to {self.source.id}")
...
class Graph:
    ...
    def execute(self, mode, num_outputs, input_tensor, reference_tensor):
        print("[GRAPH] Starting execution cycle")
...
def main(num_rounds=2500):
    ...
    for round_num in range(num_rounds):
        round_input = input_tensor.clone()
        round_reference = reference_tensor.clone()
        gen_loss, disc_loss = graph.execute(
            mode="feature",
            num_outputs=len(input_tensor),
            input_tensor=round_input,
            reference_tensor=round_reference
        )
