...
def generate_texture(binary_classifier, conv_predictor, reproducer, categorizer, input_texture, rolling_cache, device='cpu', save_path='generated_textures'):
    ...
    categorizer.eval()
    with torch.no_grad():
        input_tensor = input_texture.to(device)
        binary_output = binary_classifier(input_tensor)
        is_gaussian = binary_output.squeeze() > 0.5

        if is_gaussian:
            mu, sigma = conv_predictor(input_tensor)
    ...
    input_texture_np = input_tensor.cpu().squeeze().numpy()
    generated_np = generated.squeeze().numpy()

    # Prepare text annotations
    annotation_text = f"Is Gaussian: {is_gaussian.item():.2f}\nCategory: {predicted.item()}"

    return input_texture_np, generated_np, annotation_text

# =========================
# Main Function with Pygame Visualization
# =========================
def main():
    # Initialize Pygame
    pygame.init()
    ...
    pygame.display.set_caption("Neural Network Mosaic Visualization")
    ...
    # =========================
    def visualize_mosaic(input_img, generated_img, annotation_text, window_size=(800, 600)):
        # Convert grayscale images to RGB
        input_rgb = np.stack([input_img]*3, axis=-1)
        generated_rgb = np.stack([generated_img]*3, axis=-1)

        # Resize images to fit the window
        scale_factor = 2  # Adjust as needed
        input_rgb = (input_rgb * 255).astype(np.uint8)
        generated_rgb = (generated_rgb * 255).astype(np.uint8)
        input_surface = pygame.surfarray.make_surface(np.transpose(input_rgb, (1, 0, 2)))
        generated_surface = pygame.surfarray.make_surface(np.transpose(generated_rgb, (1, 0, 2)))

        # Create a black background
        mosaic = pygame.Surface(window_size)
        mosaic.fill((0, 0, 0))

        # Blit images onto the mosaic
        mosaic.blit(pygame.transform.scale(input_surface, (256, 256)), (50, 50))
        mosaic.blit(pygame.transform.scale(generated_surface, (256, 256)), (300, 50))
        ...
        for i, line in enumerate(lines):
            ...
            mosaic.blit(text_surface, (50, 320 + i*30))
    ...
    def run_visualization():
        ...
        # Perform inference
        input_img, generated_img, annotation_text = generate_texture(
            binary_classifier, conv_gaussian_predictor, gaussian_reproducer, categorizer,
            sample_input, rolling_cache, device=device, save_path='generated_textures'
        )

        # Create mosaic
        mosaic = visualize_mosaic(input_img, generated_img, annotation_text, window_size=window_size)
        ...
        while running:
            ...
            # Blit the mosaic onto the screen
            screen.blit(mosaic, (0, 0))
            pygame.display.flip()
    ...
    if __name__ == "__main__":
        ...
        models_exist = all([
            os.path.exists(os.path.join('models', f'binary_classifier_epoch{NUM_EPOCHS}.pth')),
            os.path.exists(os.path.join('models', f'conv_gaussian_predictor_epoch{NUM_EPOCHS}.pth')),
            os.path.exists(os.path.join('models', f'gaussian_reproducer_epoch{NUM_EPOCHS}.pth')),
            os.path.exists(os.path.join('models', f'categorizer_epoch{NUM_EPOCHS}.pth'))
        ])
        ...
        if not models_exist:
            ...
            print("Models already trained. Starting visualization...")
