...
class SceneBuilder:
    ...
    def update_dynamic_cube(self, orientation=None):
        """
        Update the dynamic cube's orientation. Position is fixed to (0,0,0).
        """
...
# PhysicsAuthority
class PhysicsAuthority(Authority):
    def __init__(self, name, state, scene_builder):
        """
        Initialize the PhysicsAuthority with a SceneBuilder.
        :param name: Unique identifier for the authority.
        :param state: Dictionary representing the current state.
        :param scene_builder: Instance of SceneBuilder.
        """
        super().__init__(name, state)
        self.scene_builder = scene_builder
        self.physics_client = p.connect(p.DIRECT)  # Use p.GUI for graphical interface
        p.setAdditionalSearchPath(pybullet_data.getDataPath())  # To load plane and other models
        p.setGravity(0, 0, 0)  # Disable gravity
        self.create_cube()

    def create_cube(self):
        """
        Create a cube rigid body in PyBullet fixed at (0,0,0).
        """
        # Define cube properties
        mass = 1  # Non-zero mass to allow rotation
        collision_shape = p.createCollisionShape(p.GEOM_BOX, halfExtents=[0.5, 0.5, 0.5])
        visual_shape = p.createVisualShape(p.GEOM_BOX, halfExtents=[0.5, 0.5, 0.5], rgbaColor=[1,0,0,1])

        # Create the cube at position (0,0,0) with no initial velocity
        ...
        # Optionally, set constraints or initial velocities here

    def update_state(self, input_data):
        """
        Step the physics simulation and update the cube's orientation.
        """
        # Step the simulation
        p.stepSimulation()

        # Get the cube's current orientation
        pos, orn = p.getBasePositionAndOrientation(self.cube_id)

        # Reset position to (0,0,0)
        p.resetBasePositionAndOrientation(self.cube_id, [0, 0, 0], orn)

        # Update the SceneBuilder with the new orientation
        self.scene_builder.update_dynamic_cube(orientation=orn)

    def __del__(self):
        """
        Clean up the PyBullet connection on deletion.
        """
        p.disconnect()
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Mitsuba Scene Renderer with PyBullet")
    ...
    # Initialize PhysicsAuthority
    physics_state = {}
    physics_authority = PhysicsAuthority("Physics", physics_state, scene_builder)
    ...
    while running:
        ...
        # Update Physics
        physics_authority.update_state({})

        # Render the scene
        visual_authority.update_state({})
        current_image = scene_builder.render_scene()
        # print(current_image.mean())  # Optional: Remove or keep for debugging
