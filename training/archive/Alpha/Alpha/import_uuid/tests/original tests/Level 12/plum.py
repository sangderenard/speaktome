    ...
    def assign_tasks(self, input_tensor):
        ...
        with self.lock:
            ...
            for node in self.gan_nodes:
                ...
                output, loss = node.process_task(task_input, target_id, focus=random.choice(["generator", "discriminator"] ))
...
class Node:
    ...
    def process_task(self, input_tensor, target_id, focus="generator"):
        ...
        if focus == "generator":
            steps = self.config["gradient_accumulation_steps_gen"]
            for _ in range(steps):
                output = self.generator(input_tensor)
                loss = self.gen_loss_fn(output, reference.produce_tracked_reference_formation(input_tensor)[0])
                loss.backward()
                loss_accumulated += loss.item()
            torch.nn.utils.clip_grad_norm_(self.generator.parameters(), self.config["gradient_clipping_gen"])
            self.gen_optimizer.step()
            self.gen_optimizer.zero_grad()

        elif focus == "discriminator":
            steps = self.config["gradient_accumulation_steps_disc"]
            for _ in range(steps):
                ...
                loss.backward()
                loss_accumulated += loss.item()
            torch.nn.utils.clip_grad_norm_(self.discriminator.parameters(), self.config["gradient_clipping_dis"])
            self.disc_optimizer.step()
            self.disc_optimizer.zero_grad()

        # Update brain with task result
        self.brain.update_best_losses(loss_accumulated / steps, output, target_id)
        self.brain.track_loss(loss_accumulated / steps)
        output_dict = {"node_id": self.id, "output": output.clone()}
        reference.add_output(output_dict)
        return output_dict, loss_accumulated / steps
...
class Graph:
    ...
    def execute_with_delegator(self, input_tensor_shape):
        ...
        for round_num in range(1):  # Example number of rounds
            input_tensor = torch.rand(input_tensor_shape)
            ...
            # for node_id, output, loss in task_outputs:
