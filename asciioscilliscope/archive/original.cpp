...
private:
    ...
    std::vector<std::string> previousBuffer; // Buffer to track previous state
    ...
    void renderDifference() {
        while (running) {
            ...
            for (size_t row = 0; row < frontBuffer.size(); ++row) {
                for (size_t col = 0; col < frontBuffer[row].size(); ++col) {
                    if (frontBuffer[row][col] != previousBuffer[row][col]) {
                        // Move cursor to the position and print the character
                        std::cout << "\x1B[" << (row + 1) << ";" << (col + 1) << "H" << frontBuffer[row][col];
                    }
                }
            }
            std::cout << std::flush;

            previousBuffer = frontBuffer; // Update previous buffer
...
public:
    FocusingGrid(int width, int height, Screen& screenRef)
        : frontBuffer(height, std::string(width, ' ')),
          backBuffer(height, std::string(width, ' ')),
          previousBuffer(height, std::string(width, ' ')),
          screen(screenRef) {}

    void start() {
        running = true;
        renderThread = std::thread(&FocusingGrid::renderDifference, this);
    }
    ...
    void updateFromScreen(int statusBlockX, int statusBlockY) {
...

        {
            ...
            backBuffer = stringBuffer; // Update back buffer
            readyToSwap = true;
        }
        cv.notify_one();
    }
