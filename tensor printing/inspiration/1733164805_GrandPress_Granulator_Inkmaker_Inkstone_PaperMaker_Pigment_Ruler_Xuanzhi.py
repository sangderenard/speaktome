...
class GrandPress:
    """
    **GrandPress**
    Core of the simulation framework, responsible for applying gradients, masks, and generating grand sheets.
    """

    def __init__(self, ruler: Granulator):
        """
        Initialize the GrandPress with a ruler.
        
        Args:
            ruler (Granulator): Handles granularity and coordinate systems.
        """
        self.ruler = ruler
    ...
    def press(self, gradient: torch.Tensor, mask: torch.Tensor, target_tensor: torch.Tensor):
        ...
        Args:
            ...
            target_tensor (torch.Tensor): Target tensor for application.
        
        Returns:
            torch.Tensor: Updated tensor after pressing.
        """
    ...
    **Inkmaker**
    ...
    def generate_gradient(self, shape: Tuple[int, int], gradient_type: str = "linear"):
        ...
        Returns:
            torch.Tensor: Gradient tensor.
        """
        return self.granulator.translate_coordinates(
            torch.linspace(0, 1, shape[1]).repeat(shape[0], 1), scale_factor=1
        )
    ...
    def generate_noise(self, shape: Tuple[int, int], noise_type: str = "white"):
        ...
        if noise_type == "pink":
            ...
            freqs = torch.fft.fftfreq(shape[1]).unsqueeze(0).expand(*shape)
    ...
    def full_simulation(self, paper: Xuanzhi, pigment: str, gradient_type: str = "radial"):
        ...
        gradient = self.generate_gradient(sheet.shape[-2:], gradient_type)
        mask = self.generate_noise(sheet.shape[-2:], noise_type="pink")
